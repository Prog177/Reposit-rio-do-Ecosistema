    ...
    if t != 0 and num_meses == 0:
        for Iteration in range(0, int(num_meses) + int(t)+1):        
            if Iteration == 0:
                ...
                Month+=1
        for Iteration2 in range(t, int(num_meses) + int(t)+1): # Nesta fase decidimos que se o utlizador pedi-se uma simulação de 0 messes o programa mostraria como output o último mês p'ra pois quando se pede 0 meses na realidade pede-se o último mês simulado anteriormente pois é o nºde messes simulado anteriormente mais o 0, que resulta no último mês simulado
            if Iteration2==0:
