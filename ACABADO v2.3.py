...
def simular_populacoes (num_meses, param, t):
    ...
    AlternateListCo = []
    AlternateListLo = []
    if t != 0 and num_meses == 0: # Utilizamos esta condicional p'ra especificar o que acontece quando é a segunda simulção e o número de mesas a simular é zero
        for Iteration in range(0, int(num_meses) + int(t)+1):        
            if Iteration == 0:
                ...
                Iteration -= 1 #Esta mudança no valor do iteration serve p'ra que os paramêtros das funções invocadas p'ra simular as populações sejam o da iteração anterior. Logo como foi indicado nas Etapas 1-3 as simulaçõesd das populações são feitas com os valores (t-1) neste caso(Iteration-1)
                ...
                Iteration+=1# Aqui retomanos o valor inicial de Iteration p'ra definir os novos valores no dicionário param 
                ...
                Month+=1
        for Iteration2 in range(t, int(num_meses) + int(t)+1): # Nesta fase decidimos que se o utlizador pedi-se uma simulação de 0 meses após ter havido simulação prévia o programa mostraria como output o último mês p'ra pois quando se pede 0 meses na realidade pede-se o último mês simulado anteriormente pois é o nºde messes simulado anteriormente mais o 0, que resulta no último mês simulado
            if Iteration2==0:
    ...
    while True:
        if Resposta == 's':
            ...
            if Resposta1.isdigit():
                ...
                valores.append(Resposta1)
                return Months, valores
            else:
                if Resposta1 == 'a':
                    return menu()
...
def simulador(nome_ficheiro):
    ...
    while x[-1] == 0:
        FuntionX, valores = menu()
        x.append(1)
        t = 0
    while True:
        if FuntionX[-2] == 's':
            if FuntionX[-1] == 'q':
                        nao_necessario, cenouras, coelhos, lobos, param, t = simular_populacoes(int(valores[-1]), DicEco, t)
                ...
                FuntionX, valores = menu()
        if FuntionX[-2] == 'd':
                FinalAnswer, cenouras, coelhos, lobos, param, t = simular_populacoes(valores[-1],DicEco, t)
                if FuntionX[-1] == 'c':
